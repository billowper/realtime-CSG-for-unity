	...
	internal sealed class SceneDragToolMaterials : ISceneDragTool
	{
		...
		public bool ValidateDrop(SceneView sceneView)
		{
			...
			foreach (var obj in DragAndDrop.objectReferences)
			{
				...
				{
					var material = FindOrCreateMaterial(texture2D);
		...
		{
			var guids = AssetDatabase.FindAssets("t:material", new[] { CSGSettings.MaterialDirectory });
			...
			foreach (var guid in guids)
			{
				...
				if (material && material.HasProperty("_BaseMap") && material.GetTexture("_BaseMap") == texture2D)
				{
					return material;
				} 
			}
			
			var newMaterial = new Material(Shader.Find(CSGSettings.DefaultShader));

			newMaterial.SetTexture("_BaseMap", texture2D);

			var materialsRoot = $"Assets/{CSGSettings.MaterialDirectory}";
			...
			Debug.Log($"[RealtimeCSG]: Created material at {AssetDatabase.GetAssetPath(newMaterial)}");
